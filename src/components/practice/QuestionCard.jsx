...
import { Languages, Sparkles, Share2, Check } from 'lucide-react';
...
export default function QuestionCard({ currentQ, currentQuestionData, showQHindi, isTranslatingQ, hindiQuestionText, handleTranslateQuestion, subjectId, topicId }) {
  ...
  const handleShare = async () => {
    ...
    const shareUrl = topicId && subjectId
      ? `${window.location.origin}/quiz/${subjectId}/${topicId}?q=${currentQ + 1}`
    ...
    if (navigator.share) {
      try {
        ...
        return;
      } catch (e) { /* user cancelled, fallback to copy */ }
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-4 mb-4 sm:mb-6 relative z-10">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-60 ${showQHindi ? 'bg-orange-500/20 border-orange-500/40 text-orange-400' : 'bg-[#282e39] border-slate-600 text-slate-300 hover:bg-[#3b4354]'}`}
          >
            {isTranslatingQ
          ...
          {/* Ask Google AI - AI Overview mode */}
          <button
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all duration-300 cursor-pointer hover:scale-105 ${copied ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400' : 'bg-[#282e39] border-slate-600 text-slate-300 hover:bg-[#3b4354] hover:border-slate-500 hover:text-white'}`}
          >
