...
export default function AdminPanel() {
  ...
  // ── BULK JSON IMPORT (NO AI) ──
  const handleBulkJsonUpload = (e) => {
    const uploadedFile = e.target.files[0];
    if (!uploadedFile) return;
    
    setStatus('Importing JSON...'); setStatusType('loading');
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          setStatus(`Successfully imported ${formattedData.length} questions!`);
  ...
  const generateQuestions = async () => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert bilingual (English & Hindi) educator and content creator.
...
3. Each object must EXACTLY have these keys: "question", "questionHindi", "options", "answer", "explanation", "explanationHindi", "examReference", "geometryType", "geometryData".
4. OPTIONS: array of 4 VERY SHORT choices IN ENGLISH ONLY.
5. "answer" must exactly match the correct option string.
6. EXAM REFERENCE TAGGING: 
   ${isCurrentAffairs 
     ? 'Set "examReference" to "Recent Trend".' 
     : 'Set the "examReference" field to the exact past exam name and year (e.g., "SSC CGL 2021").'}
7. ${isAdvanced ? 'STRICT MATH/REASONING RULE: You MUST use LaTeX wrapped in $ (inline) or $$ (block) for ALL math symbols, equations, and fractions. Also, if a question requires a chart, graph, or geometric figure, set "geometryType" to a string (e.g. "recharts-pie", "svg-triangle") and provide the JSON stringified data in "geometryData". If no figure is needed, set both to null.' : 'Set "geometryType" and "geometryData" to null.'}
8. 🛑 STRICT HINDI RULE: Provide proper, accurate Hindi translations in "questionHindi" and "explanationHindi" using Devanagari script.`;
  ...
  const modifyQuestionWithAI = async (index) => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert Hindi educator. MODIFY the provided JSON question STRICTLY based on the user's instruction. Keep the output format strictly as a valid JSON object with the exact same keys (including questionHindi and geometryType). Use proper LaTeX for math.`;
  ...
  const addNewQuestion = () => {
    const newQ = {
      ...
      examReference: "Expected",
      geometryType: null, geometryData: null
    };
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-16 border-b border-[#2a2f3a] bg-[#0f1115]/80 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-white tracking-tight">AI Content Engine</h1>
            <span className="px-2 py-0.5 rounded text-xs font-medium bg-[#0d59f2]/20 text-[#0d59f2] border border-[#0d59f2]/20">Beta</span>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-sm text-slate-400 flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${statusType === 'loading' ? 'bg-yellow-500 animate-pulse' : statusType === 'error' ? 'bg-red-500' : 'bg-green-500 animate-pulse'}`}></span>
              {status}
            </div>
        ...
        <div className="flex flex-1 overflow-hidden relative z-10">
          ...
          <section className="w-[400px] h-full flex flex-col border-r border-[#2a2f3a] bg-[#111318]/50 shrink-0 overflow-y-auto p-6 gap-6 custom-scrollbar backdrop-blur-md">
            ...
            <div className="space-y-1">
              <h2 className="text-lg font-semibold text-white">Generation Parameters</h2>
              <p className="text-sm text-slate-400">Configure the AI model to generate specific exam content.</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 pl-8 border-l border-[#2a2f3a] ml-3">
                ...
                <button onClick={loadExistingQuestions} className="w-full py-2.5 mt-2 rounded-xl border border-yellow-500/30 text-yellow-500 bg-yellow-500/10 hover:bg-yellow-500/20 text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer">
                  <CloudUpload className="w-4 h-4" /> Load Existing from DB
                </button>
              </div>
            </div>
            ...
            <div className="pt-4 mt-auto space-y-3">
              <button 
                ...
                className="w-full h-12 bg-[#0d59f2] hover:bg-[#0b4ecf] disabled:opacity-50 text-white font-semibold rounded-full shadow-[0_0_20px_rgba(13,89,242,0.5)] flex items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer"
              >
              ...
              {/* BULK IMPORT BUTTON */}
              <div className="relative">
                <input type="file" ref={bulkJsonRef} className="hidden" accept=".json" onChange={handleBulkJsonUpload} />
                <button 
                  onClick={() => bulkJsonRef.current.click()}
                  className="w-full h-10 border border-slate-700 bg-[#181b21] hover:bg-[#27272a] text-slate-300 font-medium rounded-full flex items-center justify-center gap-2 transition-all text-sm cursor-pointer"
                >
                  <Upload className="w-4 h-4" /> Bulk Import JSON List
                </button>
              </div>
            </div>
          </section>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar pb-32">
              ...
              {questions.length === 0 && !isLoading && (
                <div className="flex flex-col items-center justify-center h-full opacity-50">
                  ...
                  <p className="text-slate-400">Your generated or imported questions will appear here.</p>
                </div>
              )}
                ...
                return (
                  <article key={qIndex} className="bg-[#181b21] rounded-2xl border border-[#2a2f3a] p-6 relative group transition-all hover:border-[#0d59f2]/30">
                    ...
                    <div className="absolute top-6 right-6 flex gap-2">
                      ...
                      <button onClick={() => {toggleSelect(qIndex); deleteSelected();}} className="w-8 h-8 rounded-full bg-[#111318] border border-[#2a2f3a] flex items-center justify-center text-slate-400 hover:text-red-400 hover:border-red-400 transition-colors cursor-pointer" title="Delete">
                    ...
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Question (English/LaTeX)</label>
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea value={q.questionHindi} onChange={e => handleQuestionChange(qIndex, 'questionHindi', e.target.value)} className="w-full bg-[#111318] border border-[#2a2f3a] rounded-xl p-3 text-sm text-white resize-none h-24 focus:ring-1 focus:ring-[#0d59f2] focus:border-[#0d59f2] outline-none custom-scrollbar font-display" />
                          </div>
                        </div>
                      </div>
                    </div>
                        ...
                        return (
                          <div key={optIndex} className={`flex items-center gap-3 p-2 rounded-lg transition-colors border ${isCorrect ? 'border-transparent bg-[#111318]/50' : 'border-transparent hover:border-[#2a2f3a] hover:bg-[#111318]/50'}`}>
                            <input 
                              ...
                              className="w-4 h-4 border-slate-600 bg-[#111318] text-[#0d59f2] focus:ring-offset-0 focus:ring-0 cursor-pointer"
                            />
                            ...
                            <input 
                              ...
                              placeholder={`Option ${letters[optIndex]} (Supports LaTeX)`}
                              className={`bg-transparent border-none text-sm w-full focus:ring-0 p-0 placeholder-slate-600 outline-none ${isCorrect ? 'text-white' : 'text-slate-300'}`} 
                            />
                    ...
                    <div className="pl-9 grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6 pt-4 border-t border-[#2a2f3a]">
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-green-500 uppercase tracking-wider flex items-center gap-1"><Sparkles className="w-3 h-3"/> Explanation (English)</label>
                        <textarea value={q.explanation} onChange={e => handleQuestionChange(qIndex, 'explanation', e.target.value)} className="w-full bg-[#111318] border border-green-500/20 rounded-xl p-3 text-xs text-slate-300 resize-none h-16 focus:border-green-500 outline-none custom-scrollbar" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-green-500 uppercase tracking-wider flex items-center gap-1"><Sparkles className="w-3 h-3"/> Explanation (Hindi)</label>
                        <textarea value={q.explanationHindi} onChange={e => handleQuestionChange(qIndex, 'explanationHindi', e.target.value)} className="w-full bg-[#111318] border border-green-500/20 rounded-xl p-3 text-xs text-slate-300 resize-none h-16 focus:border-green-500 outline-none custom-scrollbar font-display" />
                      </div>
                    </div>

                    {/* Geometry & Tags Section */}
                    <div className="pl-9 mt-4 flex flex-wrap items-center gap-2 border-t border-[#2a2f3a] pt-4">
                      ...
                      <div className="px-2 py-1 rounded bg-indigo-900/30 border border-indigo-500/30 flex items-center gap-2 w-full mt-2 lg:w-auto lg:mt-0">
                        ...
                        <input type="text" value={q.geometryType || ''} onChange={e => handleQuestionChange(qIndex, 'geometryType', e.target.value)} placeholder="Type (e.g. svg-triangle)" className="bg-transparent border-none p-0 text-indigo-200 focus:ring-0 outline-none w-32 text-[10px]" />
                        <input type="text" value={q.geometryData ? (typeof q.geometryData === 'string' ? q.geometryData : JSON.stringify(q.geometryData)) : ''} onChange={e => handleQuestionChange(qIndex, 'geometryData', e.target.value)} placeholder="Data (JSON String)" className="bg-transparent border-none p-0 text-indigo-200 focus:ring-0 outline-none w-48 text-[10px] border-l border-indigo-500/30 pl-2 ml-1" />
                      </div>
