...
// Icon Mapping Data file ke strings ko actual Lucide components mein badalne ke liye
const IconMap = {
...
export default function SubjectDetailPage() {
  ...
  // URL se aaye subjectId (jaise "Maths") ko lowercase karke data array me dhundhein
  const subject = subjectsData.find(s => s.id.toLowerCase() === (subjectId || '').toLowerCase());

  // Page load hone par scroll top par le jayein
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [subjectId]);

  // Agar user ne galat URL daal diya (Subject nahi mila)
  if (!subject) {
    return (
  ...
  // Dynamic Icon aur Color variables
        ...
        <div className="relative bg-[#161616]/80 backdrop-blur-md border border-white/5 rounded-[2rem] p-8 md:p-12 mb-12 overflow-hidden flex flex-col md:flex-row gap-8 items-center md:items-start shadow-xl">
          ...
          <div className="text-center md:text-left flex-1">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
              ...
              <button className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors flex items-center gap-2 shadow-[0_0_15px_-3px_rgba(59,130,246,0.5)] cursor-pointer">
        ...
        <div className="space-y-12">
          {subject.categories.map((category, index) => (
            <section key={category.id} className="relative">
              ...
              {/* Grid of Topics inside Category */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {category.topics.map((topic) => (
                  <div key={topic.id} className="topic-card rounded-2xl p-6 group flex flex-col h-full relative overflow-hidden">
                    ...
                    {topic.isNew && (
                      <div className="absolute top-4 right-4 px-2 py-0.5 rounded text-[10px] font-bold bg-blue-500/20 text-blue-400 border border-blue-500/20">
                        NEW
                      </div>
                    )}
                    ...
                    <div className="mt-auto pt-4 border-t border-white/5">
                      ...
                      >
                        Practice Now <ArrowRight className="w-4 h-4 transition-transform group-hover/btn:translate-x-1" />
                      </button>
                    </div>
